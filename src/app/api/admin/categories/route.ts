...
export async function GET(request: NextRequest) {
  try {
    ...
    return apiInternalError(error, "admin-categories:get");
  }
}
...
export async function POST(request: NextRequest) {
  try {
    const rateLimit = checkRateLimit({
      key: buildRateLimitKey(request, "admin-category-create"),
      limit: 60,
      windowMs: 60_000,
    });
    ...
    const input: TablesInsert<"categories"> = {
      name: validation.data.name,
      slug: validation.data.slug,
      short_name: validation.data.shortName,
      description: validation.data.description ?? null,
      image_media_id: validation.data.imageMediaId ?? null,
      hover_media_id: validation.data.hoverMediaId ?? null,
    ...
    return apiInternalError(error, "admin-categories:post");
  }
}
